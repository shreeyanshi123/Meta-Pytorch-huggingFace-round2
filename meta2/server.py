...
async def dashboard_co2(episode_id: str):
    """Return CO2 savings and green-code metrics for an active episode.

    Compares the episode's current file state against its original files,
    runs :class:`~environment.track_c.GreenCodeEvaluator`, and returns the
    full dashboard payload from :func:`~environment.co2_calculator.generate_dashboard_data`.

    Parameters
    ----------
    episode_id:
        The UUID returned by ``POST /reset``.

    Returns
    -------
    JSON payload with ``summary``, ``co2_estimates``, ``code_delta``, and
    ``constants_used`` sections.
    """
    if episode_id not in active_episodes:
        raise HTTPException(status_code=404, detail="Episode not found")

    ctx = active_episodes[episode_id]
    # Retrieve original files via a fresh generator observation (stored in ctx)
    orig_files: Dict[str, Any] = getattr(ctx, "_orig_files", ctx.files)
    updated_files: Dict[str, Any] = ctx.files
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Green evaluation failed: {exc}",
        )


@app.get("/health/green")
def health_green():
    """Confirm that Track C (Green-Code Optimizer) is active.

    Returns
    -------
    JSON confirming the track status and sub-components.
    """
    return {
        "track_c": "enabled",
        "graphlet_analyzer": "active",
    }
