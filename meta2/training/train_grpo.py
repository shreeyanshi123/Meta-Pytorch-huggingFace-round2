...
def reward_function(completions, prompts, files, rules_active, **kwargs):
    ...
    for idx, (completion, orig_files, active_rules) in enumerate(zip(completions, files_batch, rules_batch)):
        try:
            ...
            try:
                green_eval = GreenCodeEvaluator()
                green_score = green_eval.evaluate(orig_files, updated_files)
                score_c = green_score.total
            except Exception as _gc_err:
                score_c = 0.0
                print(f"  [Track C] skipped (error): {_gc_err}")
            print(f"  [Track C] score_c={score_c:.3f} "
                  f"(graphlet={getattr(green_score, 'graphlet_score', 0):.3f} "
                  f"cpu={getattr(green_score, 'cpu_improvement', 0):.3f} "
                  f"mem={getattr(green_score, 'memory_improvement', 0):.3f})")
            ...
            # ── Final reward: Track A + B + C weighted combination ────────────
            reward = (
                0.50 * score_a +
                0.35 * score_b +
                0.15 * score_c +
